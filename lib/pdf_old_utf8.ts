...
function getAnswerLabel(answer: string, question: Question): string {
  if (question.type === "true-false") {
    return answer === "true" ? "Verdadeiro" : answer === "false" ? "Falso" : "ÔÇö"
  }
  if (question.type === "discursive") return answer || "ÔÇö"
  const choice = question.choices.find((c) => c.id === answer)
  return choice ? choice.text : "ÔÇö"
}
...
function getCorrectLabel(question: Question): string {
  ...
  if (question.type === "discursive") return "Quest├úo discursiva ÔÇö corre├º├úo manual"
  const choice = question.choices.find((c) => c.id === question.correctAnswer)
  return choice ? choice.text : "ÔÇö"
}

function typeLabel(type: Question["type"]): string {
  if (type === "multiple-choice") return "M├║ltipla Escolha"
...
export function printStudentPDF({ submission, assessment, questions }: PDFData): void {
  ...
  const rows = orderedQuestions
    .map((q, i) => {
      ...
      const studentLabel = studentAns ? getAnswerLabel(studentAns.answer, q) : "ÔÇö"
            ...
            .map(
              (c) =>
                `<li style="margin:2px 0;padding:3px 6px;border-radius:4px;font-size:12px;
                    ...
                    ${c.text}${c.id === q.correctAnswer ? " Ô£ô" : ""}${c.id === studentAns?.answer && !isCorrect ? " Ô£ù" : ""}
                    </li>`
            )
            .join("")}
             </ul>`
        ...
        <div style="margin-bottom:16px;padding:12px;border:1px solid #e5e7eb;border-radius:8px;break-inside:avoid;">
          <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px;">
            <span style="font-size:11px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:.5px;">
              Quest├úo ${i + 1} &nbsp;┬À&nbsp; ${typeLabel(q.type)} &nbsp;┬À&nbsp; ${assessment.pointsPerQuestion} pt${assessment.pointsPerQuestion !== 1 ? "s" : ""}
            </span>
          ...
          ? `<div style="margin-top:6px;display:flex;gap:8px;">
                  <span style="padding:3px 10px;border-radius:4px;font-size:12px;background:${studentAns?.answer === "true" && !isCorrect ? "#fee2e2" : studentAns?.answer === "true" ? "#dcfce7" : "#f3f4f6"};color:${studentAns?.answer === "true" && !isCorrect ? "#991b1b" : studentAns?.answer === "true" ? "#166534" : "#374151"}">Verdadeiro${q.correctAnswer === "true" ? " Ô£ô" : ""}${studentAns?.answer === "true" && !isCorrect ? " Ô£ù" : ""}</span>
                  <span style="padding:3px 10px;border-radius:4px;font-size:12px;background:${studentAns?.answer === "false" && !isCorrect ? "#fee2e2" : studentAns?.answer === "false" ? "#dcfce7" : "#f3f4f6"};color:${studentAns?.answer === "false" && !isCorrect ? "#991b1b" : studentAns?.answer === "false" ? "#166534" : "#374151"}">Falso${q.correctAnswer === "false" ? " Ô£ô" : ""}${studentAns?.answer === "false" && !isCorrect ? " Ô£ù" : ""}</span>
                 </div>`
          : ""
        }
          ${!isDiscursive
  ...
  const displayInstitution = (!assessment.institution || assessment.institution.includes("ENSINO TEOL├ôGICO")) ? "Instituto B├¡blico das Assembl├®ias de Deus" : assessment.institution
  const displayProfessor = (!assessment.professor || assessment.professor.includes("F├íbio Barreto") || assessment.professor === "Professor") ? "Corpo Docente" : assessment.professor

  const html = `<!DOCTYPE html>
<html lang="pt-BR">
<head>
  <meta charset="UTF-8"/>
  <title>Prova ÔÇö ${submission.studentName}</title>
...
export function printAnswerKeyPDF({ assessment, questions }: { assessment: Assessment, questions: Question[] }): void {
  ...
  const rows = orderedQuestions.map((q, i) => {
    ...
    return `<div style="margin-bottom:16px;padding:12px;border:1px solid #e5e7eb;border-radius:8px;break-inside:avoid;">
      <strong>Quest├úo ${i + 1}</strong>: ${q.text}<br/>
      <span style="color: #166534; font-weight: bold;">Gabarito: ${correctLabel}</span>
    </div>`
  }).join("")
  const html = `<!DOCTYPE html><html><head><title>Gabarito ÔÇö ${assessment.title}</title></head><body><h1>${assessment.title} - Gabarito</h1>${rows}</body></html>`
...
export function printCurriculumPDF(semesters: Semester[], disciplines: Discipline[]): void {
  const semestersHtml = semesters.sort((a,b) => a.order - b.order).map(s => {
    ...
    const rows = sDisciplines.map(d => `<tr><td>${d.name}</td><td>${d.professorName || 'N├úo atribu├¡do'}</td></tr>`).join('')
...
export function printGradesReportPDF(grades: StudentGrade[], disciplineName: string): void {
  ...
  const html = `<!DOCTYPE html><html><head><title>Di├írio de Notas</title></head><body><h1>Di├írio de Notas: ${disciplineName}</h1><table border="1" width="100%"><thead><tr><th>ALUNO</th><th>PROVA</th><th>TRABALHO</th><th>SEMIN├üRIO</th><th>M├ëDIA</th><th>SITUA├ç├âO</th></tr></thead><tbody>${rows}</tbody></table></body></html>`
  const win = window.open("", "_blank"); if (win) { win.document.write(html); win.document.close(); win.onload = () => win.print() }
}
...
export function printAttendanceReportPDF(attendances: Attendance[], students: StudentProfile[], disciplineName: string): void {
  ...
  const html = `<!DOCTYPE html><html><head><title>Relat├│rio de Frequ├¬ncia</title></head><body><h1>Folha de Frequ├¬ncia: ${disciplineName}</h1><table border="1" width="100%"><thead><tr><th>ALUNO</th><th>PRESEN├çAS</th><th>% FREQU├èNCIA</th></tr></thead><tbody>${rows}</tbody></table></body></html>`
  const win = window.open("", "_blank"); if (win) { win.document.write(html); win.document.close(); win.onload = () => win.print() }
}
...
export function printStudentListPDF(students: StudentProfile[], classes: any[]): void {
  const rows = students.map((s, i) => {
    ...
    return `<tr><td>${i + 1}</td><td>${s.name}</td><td>${s.enrollment_number || "ÔÇö"}</td><td>${cls?.name || "Sem Turma"}</td><td>${s.phone || "ÔÇö"}</td></tr>`
  }).join('')
  const html = `<!DOCTYPE html><html><head><title>Relat├│rio de Alunos</title></head><body><h1>Relat├│rio Geral de Alunos</h1><table border="1" width="100%"><thead><tr><th>#</th><th>Nome</th><th>Matr├¡cula</th><th>Turma</th><th>Telefone</th></tr></thead><tbody>${rows}</tbody></table></body></html>`
  const win = window.open("", "_blank"); if (win) { win.document.write(html); win.document.close(); win.onload = () => win.print() }
}
...
export function printBlankAssessmentPDF(assessment: Assessment, questions: Question[]): void {
    ...
    const rows = orderedQuestions.map((q, i) => {
        ...
        return `
          <div style="margin-bottom:24px;break-inside:avoid;padding:12px;border-bottom:1px dashed #eee;">
              <div style="font-weight:bold;margin-bottom:6px;">Quest├úo ${i+1} (${assessment.pointsPerQuestion} pts)</div>
    ...
    <body onload="window.print()">
      <div style="text-align:center;border-bottom:2px solid #000;padding-bottom:15px;margin-bottom:25px;">
          <h2 style="margin:0;">${assessment.institution || "IBAD - Instituto B├¡blico das Assembl├®ias de Deus"}</h2>
...
export function printOverviewPDF(assessment: Assessment, submissions: StudentSubmission[], questions: Question[]): void {
    ...
    const html = `<!DOCTYPE html><html><head><title>Relat├│rio: ${assessment.title}</title><style>body{font-family:sans-serif;padding:30px;}</style></head>
    <body onload="window.print()">
      <h1 style="color:#1e3a5f;border-bottom:2px solid #f97316;padding-bottom:10px;">Resumo Acad├¬mico: ${assessment.title}</h1>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:30px;background:#f8fafc;padding:20px;border-radius:10px;">
          ...
          <div>
            <strong>M├®dia da Turma:</strong> ${avg.toFixed(1)} / ${assessment.totalPoints} (${((avg/assessment.totalPoints)*100).toFixed(0)}%)
          </div>
      </div>
