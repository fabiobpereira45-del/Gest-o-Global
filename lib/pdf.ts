    ...
    const header = `
      <div style="border:1px solid #cbd5e1;border-radius:8px;padding:16px;margin-bottom:24px;">
        <div style="display:flex;align-items:center;gap:16px;margin-bottom:12px;">
          <img src="${assessment.logoBase64 || IBAD_LOGO}" style="max-width:80px;max-height:80px;object-fit:contain;" alt="Logo"/>
          <div style="flex:1;">
    ...
    return `
      <div style="${versionIdx > 0 ? 'page-break-before: always;' : ''}">
        ...
        <div style="margin-top:40px;padding-top:12px;border-top:1px solid #e5e7eb;font-size:10px;color:#9ca3af;text-align:center;">
          Documento gerado pelo Sistema de Avaliações IBAD — Bom desempenho!
        </div>
      </div>
    `
  }).join("\n")
...
<head>
  <meta charset="UTF-8"/>
  <title>Relatório de Visão Geral — IBAD</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
...
<body>
  <div style="text-align:center;margin-bottom:40px;">
    <img src="${IBAD_LOGO}" style="height:80px;width:auto;margin-bottom:15px;" alt="Logo IBAD" />
    <div style="font-size:12px;font-weight:700;color:#f97316;text-transform:uppercase;letter-spacing:1px;margin-bottom:4px;">IBAD — Núcleo Cosme de Fárias</div>
  ...
  <div style="margin-top:60px;padding-top:20px;border-top:1px solid #e5e7eb;font-size:11px;color:#9ca3af;text-align:center;">
    Documento oficial do Sistema de Gestão Acadêmica IBAD
  </div>
</body>
</html>`
...
<body>
  <div style="max-width:800px;margin:0 auto;background:#fff;padding:40px;border-radius:16px;box-shadow:0 4px 6px -1px rgba(0,0,0,0.1);">
    ...
    <div style="border-bottom:4px solid #1e3a5f;padding-bottom:20px;margin-bottom:30px;text-align:center;">
      <img src="${IBAD_LOGO}" style="height:70px;width:auto;margin-bottom:10px;" alt="Logo IBAD" />
      <div style="font-size:12px;font-weight:800;color:#f97316;text-transform:uppercase;letter-spacing:2px;margin-bottom:8px;">${assessment.institution || "IBAD"}</div>
    ...
    <div style="margin-top:40px;padding-top:20px;border-top:1px solid #e5e7eb;font-size:12px;color:#9ca3af;text-align:center;">
      Documento gerado pelo Sistema de Avaliações IBAD — Página 1 de 1
    </div>
  </div>
</body>
</html>`
...
<body>
  <div style="border-bottom: 4px solid #1e3a5f; padding-bottom: 20px; margin-bottom: 30px; display: flex; align-items: flex-end; gap: 20px;">
    <img src="${IBAD_LOGO}" style="height:60px;width:auto;" alt="Logo IBAD" />
    <div style="flex: 1; display: flex; justify-content: space-between; align-items: flex-end;">
      <div>
        <div style="font-size: 12px; font-weight: 800; color: #f97316; text-transform: uppercase; letter-spacing: 2px; margin-bottom: 4px;">${assessment.institution || "IBAD"}</div>
  ...
  <div style="margin-top: 40px; padding-top: 20px; border-top: 1px solid #e5e7eb; font-size: 11px; color: #9ca3af; text-align: center;">
    Documento gerado pelo Sistema de Avaliações IBAD — Gerado em ${new Date().toLocaleString("pt-BR")}
  </div>
</body>
</html>`
...
export function printCurriculumPDF(semesters: Semester[], disciplines: Discipline[]): void {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Grade Curricular - IBAD</title><style>body{font-family: Arial, sans-serif; padding: 40px; color: #334155;} @media print { body { padding: 0; } }</style></head>
  <body>
    <div style="text-align: center; margin-bottom: 40px; border-bottom: 4px solid #1e3a5f; padding-bottom: 20px;">
      ...
      <p style="margin: 5px 0 0 0; color: #64748b;">IBAD — Núcleo Cosme de Fárias</p>
    </div>
    ${semestersHtml}
...
export function printProfessorsPDF(professors: ProfessorAccount[], assignments: ProfessorDiscipline[], disciplines: Discipline[]): void {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Corpo Docente - IBAD</title><style>body{font-family: Arial, sans-serif; padding: 40px;} @media print { body { padding: 0; } }</style></head>
  <body>
    <div style="border-bottom: 4px solid #1e3a5f; padding-bottom: 10px; margin-bottom: 20px; display: flex; align-items: center; gap: 20px;">
      <img src="${IBAD_LOGO}" style="height:60px;width:auto;" alt="Logo IBAD" />
  ...
  <body>
    <div style="border-bottom: 5px solid #1e3a5f; padding-bottom: 15px; margin-bottom: 25px; display: flex; justify-content: space-between; align-items: flex-end;">
      <div style="display: flex; align-items: center; gap: 20px;">
        <img src="${IBAD_LOGO}" style="height:70px;width:auto;" alt="Logo IBAD" />
        <div>
          <div style="font-size: 12px; font-weight: bold; color: #f97316; text-transform: uppercase; margin-bottom: 4px;">Instituto de Ensino Teológico — IBAD</div>
...
export function printGradesReportPDF(grades: StudentGrade[], disciplineName: string): void {
  ...
  <body>
    <div style="border-bottom: 4px solid #1e3a5f; padding-bottom: 15px; margin-bottom: 20px; display: flex; align-items: center; gap: 20px;">
      <img src="${IBAD_LOGO}" style="height:50px;width:auto;" alt="Logo IBAD" />
...
export function printTeacherPaymentReceipt(data: { 
  ...
  const institution = "IBAD — Instituto Bíblico das Assembleias de Deus"
  const nucleo = "Núcleo Cosme de Fárias"
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; color: #1e293b; background: #fff; padding: 40px; line-height: 1.6; }
    .receipt-container { max-width: 800px; margin: 0 auto; border: 2px solid #e2e8f0; border-radius: 16px; padding: 40px; position: relative; overflow: hidden; }
    .receipt-header { border-bottom: 2px solid #7c3aed; padding-bottom: 20px; margin-bottom: 30px; display: flex; justify-content: space-between; align-items: center; }
    .logo-section { display: flex; align-items: center; gap: 15px; }
    .logo-placeholder { width: 60px; height: 60px; background: #7c3aed; border-radius: 12px; display: flex; align-items: center; justify-content: center; color: #fff; font-weight: 800; font-size: 20px; }
    .inst-name { font-size: 14px; font-weight: 800; color: #7c3aed; text-transform: uppercase; letter-spacing: 1px; }
    .nucleo-name { font-size: 12px; color: #64748b; font-weight: 600; }
    .receipt-title { font-size: 28px; font-weight: 900; color: #1e1b4b; margin-top: 5px; }
    .receipt-number { font-size: 14px; color: #94a3b8; font-family: monospace; }
    .amount-box { background: #f5f3ff; border: 2px solid #ddd6fe; border-radius: 12px; padding: 15px 25px; display: inline-block; font-size: 24px; font-weight: 800; color: #5b21b6; margin-bottom: 30px; }
    .content-text { font-size: 16px; color: #334155; margin-bottom: 40px; text-align: justify; }
    .content-text b { color: #1e1b4b; }
    .footer-section { margin-top: 60px; display: flex; justify-content: space-between; gap: 40px; }
    .signature-line { flex: 1; border-top: 1px solid #cbd5e1; padding-top: 10px; text-align: center; font-size: 12px; color: #64748b; font-weight: 600; }
    .copy-flag { position: absolute; top: 20px; right: -40px; transform: rotate(45deg); background: #f1f5f9; color: #94a3b8; padding: 5px 40px; font-size: 10px; font-weight: 800; text-transform: uppercase; }
    @media print { body { padding: 0; } .receipt-container { border: 1px solid #ccc; } }
  </style>
</head>
<body>
  <div class="receipt-container">
    <div class="copy-flag">Original</div>
    
    <div class="receipt-header">
      <div class="logo-section">
        ${data.logoUrl ? `<img src="${data.logoUrl}" style="max-width:70px;max-height:70px;object-fit:contain;" />` : `<div class="logo-placeholder">IBAD</div>`}
        <div>
          <div class="inst-name">${institution}</div>
          <div class="nucleo-name">${nucleo}</div>
        </div>
      </div>
      <div style="text-align: right;">
    ...
    <div style="text-align: right; margin-bottom: 20px;">
      <div class="amount-box">R$ ${data.amount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</div>
    </div>

    <div class="content-text">
      Recebi(emos) de <b>${institution} — ${nucleo}</b>, a importância de <b>R$ ${data.amount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</b> referente a: <br/><br/>
      <span style="font-size: 18px; color: #1e1b4b; display: block; border-left: 4px solid #7c3aed; padding-left: 15px;">${data.description}</span>
      <br/>
      pelo que firmo(amos) o presente recibo dando plena e geral quitação.
    </div>

    <div style="margin-bottom: 60px; font-weight: 600; color: #64748b;">
      Data: ${new Date(data.date).toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' })}
    </div>

    <div class="footer-section">
      <div class="signature-line">
        ASSINATURA DO EMISSOR<br/>
        ${institution}
      </div>
      <div class="signature-line">
        ASSINATURA DO PROFESSOR(A)<br/>
        ${data.professorName}
      </div>
    </div>
  </div>

  <div style="margin-top: 40px; border-top: 1px dashed #cbd5e1; padding-top: 40px;"></div>

  <div class="receipt-container" style="margin-top: 40px;">
    <div class="copy-flag">Cópia do Arquivo</div>
    <!-- Repetir conteúdo para via do núcleo se desejado, mas vamos manter uma única via profissional para o PDF -->
  </div>
</body>
</html>`
...
export function printStudentListPDF(students: StudentProfile[], classes: any[], filterClass: string, filterPayment: string): void {
  const rows = students.map((s, i) => {
    const cls = classes.find(c => c.id === s.class_id)
    const statusFin = s.payment_status === "paid" ? "Pago" : s.payment_status === "bolsa100" ? "Bolsa 100%" : s.payment_status === "bolsa50" ? "Bolsa 50%" : "Pendente"
    return `
      <tr style="border-bottom: 1px solid #eee;">
        <td style="padding: 10px; font-size: 11px; text-align: center;">${i + 1}</td>
        <td style="padding: 10px; font-size: 12px; font-weight: 600;">${s.name}</td>
        <td style="padding: 10px; font-size: 11px; font-family: monospace;">${s.enrollment_number || "—"}</td>
        <td style="padding: 10px; font-size: 11px;">${cls?.name || "Sem Turma"}</td>
        <td style="padding: 10px; font-size: 11px; font-weight: 700; color: ${s.payment_status === 'paid' ? 'green' : (s.payment_status || '').startsWith('bolsa') ? 'blue' : 'orange'}">${statusFin}</td>
        <td style="padding: 10px; font-size: 11px;">${s.phone || "—"}</td>
      </tr>
    `
  }).join('')
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Relatório de Alunos</title><style>body{font-family: Arial, sans-serif; padding: 30px;} @media print { body { padding: 0; } }</style></head>
  <body>
    <div style="border-bottom: 5px solid #1e3a5f; padding-bottom: 15px; margin-bottom: 25px; display: flex; align-items: center; gap: 20px;">
      <img src="${IBAD_LOGO}" style="height:70px;width:auto;" alt="Logo IBAD" />
      <div style="flex: 1;">
        <div style="font-size: 12px; font-weight: bold; color: #f97316; text-transform: uppercase;">IBAD — Núcleo Cosme de Fárias</div>
        <h1 style="margin: 0; color: #1e3a5f; font-size: 24px;">Relatório Geral de Alunos</h1>
        <div style="font-size: 12px; color: #64748b; margin-top: 5px;">
          Filtros: Turma <b>${classLabel}</b> | Financeiro <b>${payLabel}</b> | Total: <b>${students.length}</b>
        </div>
      </div>
    </div>
    <table style="width: 100%; border-collapse: collapse;">
      <thead><tr style="text-align: left; background: #f1f5f9; border-bottom: 3px solid #1e3a5f;">
        <th style="padding: 12px; font-size: 10px; text-transform: uppercase; text-align: center;">#</th>
        <th style="padding: 12px; font-size: 10px; text-transform: uppercase;">Nome</th>
        <th style="padding: 12px; font-size: 10px; text-transform: uppercase;">Matrícula</th>
        <th style="padding: 12px; font-size: 10px; text-transform: uppercase;">Turma</th>
        <th style="padding: 12px; font-size: 10px; text-transform: uppercase;">Financeiro</th>
        <th style="padding: 12px; font-size: 10px; text-transform: uppercase;">Telefone</th>
      </tr></thead>
      <tbody>${rows}</tbody>
    </table>
    <div style="margin-top: 40px; text-align: center; font-size: 11px; color: #94a3b8; border-top: 1px solid #eee; padding-top: 20px;">
      Documento gerado pelo Sistema de Gestão Acadêmica IBAD — ${new Date().toLocaleDateString('pt-BR')}
    </div>
  </body></html>`
